...
public class MapGraph {
			...
			for (Row row : sheet) {
				...
				for (Cell cell : row) {
					if (cell.getColumnIndex() == 0)
						locName = cell.getStringCellValue();
					else if (cell.getColumnIndex() == 1)
						coords[0] = cell.getNumericCellValue();
					else if (cell.getColumnIndex() == 2)
						coords[1] = cell.getNumericCellValue();
				}
				if (locName != null)
					places.put(locName, new Location(coords[0], coords[1], locName));
			}
			fis.close();
		} catch (Exception e) {
		...
		while (!pq.isEmpty()) {
			...
			for (Edge edge : loc.neighbors) {
				...
				if (neighbor.visited == false) {
					...
					if (isTimeDist)
						newDistance = loc.distance + edge.timeCost;
					else
						newDistance = loc.distance + edge.distCost;
	...
	private class Location implements Comparable<Location> {
		...
		public double distanceCalculator(double[] Loc2) {
			...
			// Assumes latitude is stored in first slot in double[] ex Loc2 =
			// [Latitude,Longitude]
			...
			double haversine = Math.pow(Math.sin(distLat / 2), 2)
					+ Math.pow(Math.sin(distLon / 2), 2) * Math.cos(lat1Radians) * Math.cos(lat2Radians); // haversine
																											// formula
																											// used from
																											// coordinates
		...
		public int timeCalculator(double distance) {
			...
			if (distance <= 99)
				averageSpeed = 40;
			else if (distance >= 51 && distance <= 99)
				averageSpeed = 50;
			else
				averageSpeed = 60;
